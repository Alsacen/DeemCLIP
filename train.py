...
class EmoCls(nn.Module):
    def __init__(self):
        ...
        self.hidden = nn.Sequential(
            nn.Linear(inpt, dim1), nn.ReLU(),
            # nn.Linear(dim1, dim2), nn.ReLU(),
            # nn.Linear(dim2, oupt))  # 此处不能加任何激活否则不收敛

            nn.Linear(dim1, oupt))  # 此处不能加任何激活否则不收敛
...
def main():
    ...
    fusion_model_emo = visual_prompt(config.network.sim_header, clip_state_dict, config.data.num_segments)
    model_text = TextCLIP(model)
    model_image = ImageCLIP(model)
    model_emo = EmoCls()  ## new
    #
    model_text = torch.nn.DataParallel(model_text).cuda()
    model_image = torch.nn.DataParallel(model_image).cuda()
    model_emo = torch.nn.DataParallel(model_emo).cuda()   ## new
    fusion_model = torch.nn.DataParallel(fusion_model).cuda()
    fusion_model_emo = torch.nn.DataParallel(fusion_model_emo).cuda()  ## new
    #wandb.watch(model)
    ...
    if device == "cpu":
        model_text.float()
        model_image.float()
    else:  # fusion_model内部计算时会统一用fp32运算再转回原精度类型，所以不需要调用convert_weights
        clip.model.convert_weights(model_text) # Actually this line is unnecessary since clip by default already on float16
        clip.model.convert_weights(model_image)
        # clip.model.convert_weights(model_emo)  ## new

    loss_img = KLLoss()
    loss_txt = KLLoss()
    loss_emo = nn.CrossEntropyLoss()  ## new
    ...
    num_text_aug, text_dict, classes_emo, text_dict_emo = text_prompt(train_data)
    # classes = text_prompt_classes(text_dict)
    optimizer = _optimizer(config, model, fusion_model)
    lr_scheduler = _lr_scheduler(config, optimizer)

    best_prec1 = 0.0
    if config.solver.evaluate:
        prec1 = validate(start_epoch,val_loader, classes_emo, device, model,fusion_model, config,num_text_aug)
        return
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        for kkk,(images,list_id) in enumerate(tqdm(train_loader)):
            ...
            texts = torch.stack([text_dict_emo[j][i,:] for i,j in zip(list_id,text_id)])
            ...
            emo_text = [text_dict[j][i] for i,j in zip(list_id,text_id)]  ## new
            emo_texts_label = emo_label(emo_text).to(device)  ## new

            image_embeddings = model_image(images)
            image_embeddings = image_embeddings.view(b,t,-1)
            image_embedding = fusion_model(image_embeddings)

            image_embedding_emo = fusion_model_emo(image_embeddings)  ## new
            image_embedding_emo = model_emo(image_embedding_emo.type(torch.float32))  ## new
            loss_emos = loss_emo(image_embedding_emo, emo_texts_label)  ## new
            ...
            total_loss = (loss_imgs + loss_texts + loss_emos)/3  ## new
        ...
        if epoch % config.logging.eval_freq == 0:  # and epoch>0
            prec1 = validate(epoch,val_loader, classes_emo, device, model, fusion_model, config, num_text_aug)
